...
function setupPlanControls() {
  ...
  startBtn.addEventListener('click', async () => {
    try {
      ...
      currentPlan = updated;
      console.log('After start, isActive=', currentPlan.isActive); // now defined
      window.location.reload();
    } catch {
      showError('Failed to start plan.');
    }
  });
  ...
  pauseBtn.addEventListener('click', async () => {
    try {
      ...
      showError('Failed to pause plan.');
    }
  });
    ...
    showModal(
      ...
      async () => {
        try {
          ...
          showError('Failed to complete plan.');
        }
      }
    );
  });
  ...
  goal.activities.forEach(act => {
    ...
    if (act.state === 'PLANNED') {
      actions.appendChild(createActionButton('Start', 'primary', async () => {
        try { await HealthPlanApi.startActivity(act.id); window.location.reload(); }
        catch { showError('Failed to start activity.'); }
      }));
    } else if (act.state === 'ACTIVE') {
      actions.appendChild(createActionButton('Complete', 'success', async () => {
        try { await HealthPlanApi.completeActivity(act.id); window.location.reload(); }
        catch { showError('Failed to complete activity.'); }
      }));
      actions.appendChild(createActionButton('Pause', 'warning', async () => {
        try { await HealthPlanApi.pauseActivity(act.id); window.location.reload(); }
        catch { showError('Failed to pause activity.'); }
      }));
    } else if (act.state === 'PAUSED') {
      actions.appendChild(createActionButton('Resume', 'primary', async () => {
        try { await HealthPlanApi.startActivity(act.id); window.location.reload(); }
        catch { showError('Failed to resume activity.'); }
      }));
    }
    ...
    actions.appendChild(createActionButton('<i class="fas fa-trash"></i>', 'danger', () => {
      showModal(
        ...
        `Remove "${act.name}"?`,
        async () => {
          try { await HealthPlanApi.deleteActivity(act.id); window.location.reload(); }
          catch { showError('Failed to delete activity.'); }
        }
      );
    }));
...
function showAddGoalModal() {
  ...
  showModal('Add Goal', html, async () => {
    ...
    try {
      ...
      window.location.reload();
    } catch { showError('Failed to add goal.'); }
  });
}
...
function showAddActivityModal(goalId) {
  ...
  showModal('Add Activity', html, async () => {
    ...
    try {
      ...
      window.location.reload();
    } catch { showError('Failed to add activity.'); }
  });
}
...
function showEditActivityModal(act) {
  ...
  showModal('Edit Activity', html, async () => {
    ...
    try {
      ...
      window.location.reload();
    } catch { showError('Failed to update activity.'); }
  });
}
